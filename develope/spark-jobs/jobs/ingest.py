...
SteamSpy `all` API를 페이지네이션으로 순회해 게임별 소유자/리뷰 추정치를
raw로 적재한다 (old_games_pipeline 전용, "옛 명작 발굴", 월간 배치).
...
과거에는 POOL 환경변수로 이 파일이 "최근작 발굴"(recent_games_pipeline,
pool="recent") 경로도 함께 처리했다 - Steam Store 검색(평가 좋은 순)으로 최근
출시작 appid를 찾고 SteamSpy appdetails를 개별 조회하는 방식. Steam 비공식
검색 API의 released 날짜 필터가 서버 쪽에서 완전히 무시되는 구조적 한계로
느리고(수십 분) 히트율이 낮아(수십 개 중 1~2개) 이 접근 자체를 폐기했다
(상세 경위: `doc/recent-games-discovery-investigation.md`,
`doc/decision-record-2026-08-19-three-section-restructure.md`). 신작 소개는
RSS 뉴스 섹션(게임메카 등)으로 대체됐고, 이 파일은 이제 old pool 경로만 담당한다.
"""
import json
import os
import time
