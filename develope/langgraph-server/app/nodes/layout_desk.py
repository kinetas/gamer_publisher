...
(dry_run이 아니면) postgres에 저장 + chromadb에 인덱싱한다.
...
async def layout_desk(state: ReportState) -> dict:
    ...
    if state.get("dry_run"):
        logs.append("[layout_desk] dry_run=True, postgres/chroma 저장 스킵")
    ...
    entries = [
        {
            ...
            "developer": item["game"].get("developer") or "",
            "text": item["final_text"],
        }
        for item in checked
    ]
    await rag.upsert_writeups(entries)
